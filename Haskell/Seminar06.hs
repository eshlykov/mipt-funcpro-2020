...
-- Haskell не имеет встроенных операторов. Все, что мы видим, определено в
...
-- Каждый оператор имеет свою ассиативность (fixity) и приоритет (predecence).
...
-- Кортеж может быть пустым, в этом случае он записывается как () (кортеэ без
...
factIf n = if n == 0 then 1 else n * factIf (n - 1)
...
factIfUnsafe n = if n >= 0
                        then factIf n
                        else error "`factorialIfUnsafe n` requires `n >= 0`"
...
-- к шаблону добавлять дополнительные условия. Эти условия называются охранными
-- выражениями (guard expressions) и записываются через вертикальную черту `|`
-- справа от шаблона. Так можно переписать функцию factorialPM с использованием
-- error:
...
-- Охранное выражение имеет тип Bool и при вычислении должно иметь значение
-- True, чтобы сопоставление прошло успешно.

-- Для одного шаблона можно указывать несколько охранных выражений, тогда
-- они рассматриваются также сверху вниз и ищется первое истинное. Перепишем
...
-- Сложность реализации линейная. Однако сведение к хвостовой рекурсии с помощью
-- простого аккумулятора позволяет сделать реализовать то же самое за линейную
-- сложность:
...
factorialShort = let
    helper acc n = if n <= 0 then acc else helper (acc * n) (n - 1)
  in helper 1
...
-- Все вместе:
--import qualified Data.Char as Char hiding (toLower)
